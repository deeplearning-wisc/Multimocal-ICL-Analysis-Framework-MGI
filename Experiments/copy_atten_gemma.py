...
class GemmaAttnInterceptorV2(nn.Module):
    def __init__(self, attn_module, layer_idx, mid_layer, alpha, beta, shared_cache, top_ratio=1.5, low_ratio=1):
        ...
        # print(type(self.attn)) #<class 'transformers.models.gemma3.modeling_gemma3.Gemma3Attention'>
        self.is_sliding = (self.attn.config.layer_types[layer_idx] == "sliding_attention")
        # 运行时状态（每个样本/批次前设置）
        self.active = True
    ...
    def forward(self, hidden_states, attention_mask=None,
        ...
        attn_weights, value_states = my_eager_attention_forward(self.attn,
            ...
            value_states,
            attention_mask,
            dropout=self.attn.attention_dropout if self.attn.training else 0.0,
        ...
        if other_spans is None:
            if self.layer_idx == self.mid_layer and (self.cache.get('other_spans') is None)  and (self.cache.get('correct_spans') is None):
                ...
                for cur_spans, q_idx in zip(spans, label_pos_list):
                    ...
                    low_thr = token_vals.mean() * self.low_ratio

                    # 大于阈值的作为 top
                    top_local = torch.nonzero(token_vals > thr, as_tuple=False).squeeze(-1)

                    # 其余的是 bottom
                    bot_local = torch.nonzero(token_vals <= low_thr, as_tuple=False).squeeze(-1)
                    ...
                    # gamma = 1.05  # 你可以尝试 2.0, 3.0 甚至更高
                    # # 1. 指数运算：大的更大，小的更小
                    # P = torch.pow(P + 1e-9, gamma) # 加 1e-9 防止0值计算出现某些梯度问题(可选)
                    # # 2. 重新归一化：保证每一行（每个Head）的和仍然为 1
                    # # 假设 P 的形状是 [H, M] 或 [H, SeqLen]，通常是在最后一个维度归一化
                    # P = P / (P.sum(dim=-1, keepdim=True) + 1e-9)
                    # attn_weights[0, :, q_idx, :] = P
            ...
            if (self.layer_idx >= self.start_layer) and (self.cache.get('other_spans') is not None)  and (self.cache.get('correct_spans') is not None):
                ...
                if "cache_position" in kwargs and kwargs["cache_position"] is not None:
                    full_len = int(kwargs["cache_position"].max().item()) + 1
                else:
                    # 兜底: 没有 cache_position 时, 对于 prefill 或无滑窗层, 直接当 full_len == K
                    full_len = K

                # 注意力窗口左端对应的全局下标
                offset = max(0, full_len - K)   # 窗口覆盖 [offset, full_len-1] 这些全局位置

                def map_global_to_local(global_idx_tensor: torch.Tensor, K: int, offset: int):
                    """global -> local 下标, 并裁掉窗口外的"""
                    if global_idx_tensor.numel() == 0:
                        return global_idx_tensor

                    local = global_idx_tensor - offset
                    mask = (local >= 0) & (local < K)
                    return local[mask]

                for cur_spans, other_spans, q_idx in zip(est_spans, est_other_spans, label_pos_list):

                    idxs_global = torch.as_tensor(cur_spans, device=attn_weights.device, dtype=torch.long)
                    idxs_global = torch.unique(idxs_global)

                    other_idxs_global = torch.as_tensor(other_spans, device=attn_weights.device, dtype=torch.long)
                    other_idxs_global = torch.unique(other_idxs_global)

                    # 如果有 label 位置, 也从 idxs 中去掉
                    if isinstance(q_idx, int):
                        q_global = torch.tensor([q_idx], device=attn_weights.device, dtype=torch.long)
                        idxs_global = idxs_global[~torch.isin(idxs_global, q_global)]

                    # 映射到窗口内的局部下标, 并去掉窗口外的
                    idxs = map_global_to_local(idxs_global, K, offset)              # [<=|idxs_global|]
                    other_idxs = map_global_to_local(other_idxs_global, K, offset)  # 同理
                    ...
                    if other_idxs.numel() > 0 and h_idx is not None:
                        t_other_idx = other_idxs.unsqueeze(0)
                        # print(P.shape)
                        # print(t_other_idx)
                        # print(h_idx )
                    ...
                    if q_local is not None:
                        threshold = 0.005
                        head_scores = P[:, q_local]  # [H] 也可以用 .mean(dim=-1)
                        top_h = 2
                        top_head_idx = torch.topk(head_scores, top_h, largest=True).indices   # [top_h]
                        h_idx = top_head_idx        # [top_h, 1]

                        P[h_idx, q_local] =  P[h_idx, q_local] * 1.2 #torch.where(P[:, q_idx] < threshold, P[:, q_idx] , P[:, q_idx])
     
                    P = P / P.sum(dim=-1, keepdim=True).clamp_min(1e-12)
                    attn_weights[0, :, -1, :] = P 
    
        else: # with grouth truth mask
            if self.layer_idx >= self.mid_layer:
                for cur_spans, other_spans, q_idx in zip(spans, other_spans, label_pos_list):
                    ...
                    if isinstance(q_idx, int) and 0 <= q_idx < K:
                        threshold = 0.005
                        P[:, q_idx] = torch.where(P[:, q_idx] < threshold, P[:, q_idx] * 2, P[:, q_idx])
...
class GemmaAttnInterceptor(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask=None,
        ...
        if self.layer_idx >= self.mid_layer:
            for q_idx in label_pos_list:
                row = attn_weights[0, :, q_idx, :]
                H, T = row.shape
                noise = torch.rand_like(row)
...
def wire_intervention(model, mid_layer=15, alpha=10, beta = 0.5, mode='flatten', top_ratio=1.5, low_ratio=1):
    """
    "flatten" or "improve"
    对同一注意力对象（id 一样）的所有引用统一替换成同一个 GemmaAttnInterceptor 实例。
    ...
    layer_idx = 0
    for attn_id, group in by_attn_id.items():
        parent0, attr0, attn0 = group[0]
        if mode=='fallten' and type(attn0).__name__ == "Gemma3Attention":
            wrapper = GemmaAttnInterceptor(attn0, layer_idx, mid_layer, alpha, shared_cache)
            for p, a, _ in group:
                setattr(p, a, wrapper)
        ...
        elif mode=="improve" and type(attn0).__name__ == "Gemma3Attention":
            wrapper = GemmaAttnInterceptorV2(attn0, layer_idx, mid_layer, alpha,  beta, shared_cache, top_ratio=top_ratio, low_ratio=low_ratio)
            for p, a, _ in group:
                setattr(p, a, wrapper)
        ...
        elif mode=='already' and type(attn0).__name__ == "Gemma3Attention":
            wrapper = attn0
             # 更新内部参数
...
def iter_interceptors(model):
    for m in model.modules():
        if isinstance(m, GemmaAttnInterceptor):
            yield m
        if isinstance(m, GemmaAttnInterceptorV2):
            yield m
...
def debug_attn_aliases(model, max_groups=8):
    ...
    for i, (aid, group) in enumerate(by_attn_id.items()):
        ...
        wrapped = ("GemmaAttnInterceptor" in types)
