...
def _restore_heatmap(v, H_eff, W_eff, t=1, reduce_mode="mean"):
    ...
    if n != t * hw:
        raise ValueError(f"len(v)={n} and t*H_eff*W_eff={t*hw} is different, cannot reshape.")
...
def cal_atten_ratio_on_diff_area(
    ...
    label_pos_list,  
    spans_per_image,        # List[Tuple[int,int]]
    patch_grids=None,       #  List[(H,W)] 
    ...
    for i, _ in enumerate(correct_mask_list):
        ...
        if patch_grids is not None and patch_grids[i] is not None:
            ...
            assert n == H * W or n % (H * W) == 0, \
                f"span_len={n} and H*W={H*W} is different."

            heat = _restore_heatmap(v, H, W, reduce_mode="mean")
        else:
            side = int(np.sqrt(len(v)))
            if side * side != len(v):
                raise ValueError("No grid info and len(v) is not a perfect square, cannot reshape to square heatmap.")
...
def main(ICL_model_name):
        ...
        for lay_id in range(L):
            ...
            if 'shape' in dataset_name:
                correct_ratio,  other_ratio = cal_atten_ratio_on_diff_area(
                    attention = attns[0],
                    correct_mask_list = mask_list[:-1], # 不计算query
    ...
    plt.plot(layers, mean_correct ,color='green', linestyle='-', label='Correct area ratio (avg)') #savgol_filter(mean_correct, 5, 2)
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(f"Rebuttal_{safe_name}_{dataset_name}_attention_ratios_per_layer.png", dpi=800)
    ...
    with open(f"Rebuttal_{safe_name}_{dataset_name}_attention_ratios_per_layer.json", "w") as f:
